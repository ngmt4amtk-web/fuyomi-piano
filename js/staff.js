import {midiToStaff,keySignature,noteLabel} from './theory.js?v=piano2';
import {CLEF_PATH,SHARP_PATH,NATURAL_PATH,NOTEHEAD_PATH} from './glyphs.js';
export {CLEF_PATH};
...
export function renderStaff({key='C',notes=[],theme='light'}={}) {
  ...
  for(const [i,n] of keySignature(key).entries())content+=path(SHARP_PATH,72+i*13,120-n.diatonic*6,10);
  notes.forEach((n,i)=>{
...
export function renderKeyboard(midi) {
  ...
  return `<svg viewBox="0 0 288 112" role="img" aria-label="${noteLabel(midi)}の鍵盤位置">${svg}<text x="6" y="108" font-size="11" fill="currentColor">${noteLabel(base)}</text><text x="280" text-anchor="end" y="108" font-size="11" fill="currentColor">ド</text></svg>`;
}
